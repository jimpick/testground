...
# Run sidecar
docker run --name testground-sidecar --detach --cap-add NET_ADMIN --cap-add SYS_ADMIN --pid host --env REDIS_HOST=testground-redis --network control --mount type=bind,source=/var/run/docker.sock,destination=/var/run/docker.sock 909427826938.dkr.ecr.us-west-2.amazonaws.com/testground:latest sidecar --runner docker
